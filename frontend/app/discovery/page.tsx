          ...
          {loading ? (
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 ${isSidebarOpen ? 'lg:grid-cols-2 xl:grid-cols-3' : 'lg:grid-cols-3 xl:grid-cols-4'} gap-6 transition-all duration-300`}>
              {campaigns.map((campaign) => {
                ...
                const estimatedReach = rate > 0 ? Math.floor((budget / rate) * 1000).toLocaleString() : "0";

                return (
                  <div key={campaign.id} className="bg-card border border-border rounded-[24px] overflow-hidden group hover:border-primary/30 transition-all shadow-sm">
                    <div className="aspect-[4/3] relative overflow-hidden bg-gradient-to-br from-primary/20 via-secondary/40 to-background">
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div className="flex items-center justify-between pt-2">
                        <p className="text-xs text-muted-foreground">
                          Expires {campaign.expires_at ? new Date(campaign.expires_at).toLocaleDateString() : "N/A"}
                        </p>
                        <Link
                          href={`/clipper?campaign_id=${campaign.id}`}
                          className="text-[11px] font-bold text-foreground uppercase tracking-wider hover:text-primary transition-colors"
                        >
                          Join
                        </Link>
                      </div>
